...
* Sistema basados em *deep learning* (DL) não aguentam o speckle.
* Filtros e classificadores baseados em DL são muito ruins para imagens SAR.
* Analisadores baseados em DL não sabem interpretar imagens SAR.
...
* A distribuição de $Z$ tem três parâmetros, e um deles "conta" quantos objetos há em cada pixel, em média:
   * As imagens SAR mais comumente usadas têm aprox. $10\times10\text{ m}^2$ de resolução espacial.
   * As frequências usadas enxergam objetos de aprox. $10\text{ cm}$.
...
* Mostramos essas estimativas, pixel a pixel, como um mapa de cores.
...
* A metodologia (entender, modelar, treinar, extrair) pode (*deve?*) ser aplicada para adaptar técnicas baseadas em *deep learning* ao problema específico.
