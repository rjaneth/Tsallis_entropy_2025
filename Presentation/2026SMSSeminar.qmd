...
$$
S = \sum_{\ell=1}^m S_\ell = \sum_{\ell=1}^m A_\ell e^{j \phi_\ell} = \underbrace{\sum_{\ell=1}^m  A_\ell \cos \phi_\ell}_{\Re(S)} +
j \underbrace{\sum_{\ell=1}^m A_\ell \sin \phi_\ell}_{\Im(S)}.
$$
We model $A_\ell\colon\Omega\rightarrow\mathbb R_+$ and $\phi_\ell\colon\Omega\rightarrow(0,2\pi]$ as random variables.
If
