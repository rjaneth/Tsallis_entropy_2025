...
subtitle: "Methodology and Application to SAR Image Analysis"
...
SAR -- Synthetic Aperture Radar:
   ...
   * Radar -- RAdio Detecting And Ranging: a sensor that operates in the microwaves 
   spectrum (radio) by emitting signals and sensing the return
   * Synthetic Aperture: a signal processing "trick" that renders fine spatial 
   resolution
...
::: {.column width="60%"}
![SAR sensor with side-looking geometry](../Figures/sar_backscatter_diagram.svg){width="100%"}
:::
...
* Complementary information to optical remote sensing
* See through clouds, fog, and rain (all-weather)
* Own source of illumination (day and night)
* Geometric and dielectric properties
* Ability to penetrate through canopies and soil
...
$$
S = \sum_{\ell=1}^m \underbrace{S_\ell}_{\in\mathbb C} = 
\underbrace{\sum_{\ell=1}^m A_\ell e^{j \phi_\ell}}_{\in \mathbb C} = \underbrace{\sum_{\ell=1}^m  A_\ell \cos \phi_\ell}_{\Re(S)} +
j \underbrace{\sum_{\ell=1}^m A_\ell \sin \phi_\ell}_{\Im(S)}.
$$
We model $A_\ell\colon\Omega\rightarrow\mathbb R_+$ and $\phi_\ell\colon\Omega\rightarrow(-\pi,\pi]$ as random variables.
If

* $m\to\infty$ (infinitely many scatterers in the resolution cell),
<!-- * $\max\operatorname{Var}(A_\ell)<\infty$ (no scatterer dominates), -->
* $A_\ell \perp A_k$ (independent amplitudes) with $\max\operatorname{Var}(A_\ell)<\infty$ (no scatterer dominates)
...
```{r Densities, out.width="50%"}
library(reshape2)
z <- seq(.01, 30, length.out=500)
dGamma <- fGammaSAR(z, 5, 2)
dGI0 <- fGI0SAR(z, 5, -1.5, 2)

dens <- melt(data.frame(z, dGamma, dGI0), measure.vars = 2:3, variable.name = "Model")

ggplot(dens, aes(x=z, y=value, col=Model)) +
...
$$
S(\mathbf Z)=  \widehat T_{\lambda}(\mathbf Z) - \Upsilon(\widehat\mu)  \approx 0 \text{ under } \mathcal H_0, \text{ and large otherwise}.
$$
...
* We improved the test with 
   * bootstrap correction, 
   * adaptive windows, and 
   * an optimized $\lambda$.
