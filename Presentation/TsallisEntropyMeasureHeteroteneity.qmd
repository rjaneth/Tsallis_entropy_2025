...
## What is "texture" in a SAR image?

::: {.incremental}
*	There is no consensus on the meaning of the term.
*	Some use it to describe a mixed sample, while others refer to the local structural organization of observations.
*	Another common usage relates to situations where the speckle is not fully developed.
...
* Each pixel value is the addition of the return from $m$ elementary backscatterers:
$$
...
$$
* Its coefficient of variation is $\operatorname{CV}(Z) = \frac{\sqrt{\operatorname{Var}(Z)}}{\mu} = \frac{1}{\sqrt{L}}$.
* [The coefficient of variation is used to check if the observations are fully-developed speckle because it does not depend on the mean. It is an intrinsic property of the model.]{.alert}
:::
...
Instead of assuming $\mu$ constant in $Z=\mu W$, we may assume that it is a random variable.
...
1. writing the Tsallis entropy as a function of the quantile function, and
1. making sensible "approximations" (estimation).
...
We want the Tsallis entropy of order $\lambda$ from an arbitrary probability density function $f$:
$$
T_\lambda = \frac{1}{1-\lambda}\biggl(1-
\int_{-\infty}^{\infty} f^\lambda(x) dx \biggr) .
$$
We transform variables:
\begin{align}
x &= Q(p), \text{ the quantile function with } 0\leq p\leq 1,\\
dx &= Q'(p) dp,\\
f(x) &= f(Q(p)), \\
f(Q(p)) &= \frac{1}{Q'(p)} \text{ (use the chain rule on }F(Q(p))=p\text{)},
\end{align}
...
T_\lambda
  ...
  \frac{1}{\lambda-1}
     ...
     \right),
$$
be bold (*a statistician*), and

::: {.incremental}
* approximate $Q(p)$ by order statistics, 
* and $Q'(p)$ by their differences (spacings).
...
## Conclusions {.fullsmall}

::: {.incremental}
* We presented a statistical approach based on Tsallis entropy to detect deviations from fully-developed speckle in intensity SAR images. 

* The test integrates bootstrap correction and adaptive windowing to balance speckle suppression and texture preservation [without requiring training data or parametric modeling]{.alert}.

* Simulated and real SAR data demonstrated that the method is well-calibrated, robust for small samples, and effective in delineating heterogeneous regions from homogeneous ones. 

* The test maintains proper control of the Type&nbsp;I error while improving detection power with increasing sample size.

* Although machine learning-based SAR classifiers are widely used, this framework offers a [statistically grounded interpretable]{.alert} significance measures and requires [no labelled data]{.alert}, making it suitable for rapid detection, screening, and scenarios with limited data availability.

* [In the same way, you can make your distribution-free test for the problem you are studying]{.alert}
:::

## Reproducibility {.orange-slide}

The manuscript and these slides are fully reproducible because

::: {.incremental}
* they were typed in Quarto, embedding
   * text and references with $\LaTeX$ and <span style="font-variant: small-caps;">Bib</span>$\TeX$,
   * R code for functions definitions, simulations, optimisations, plots,
   * extracted tables
* Everything is available at [https://github.com/rjaneth/Tsallis_entropy_2025](https://github.com/rjaneth/Tsallis_entropy_2025).
:::
