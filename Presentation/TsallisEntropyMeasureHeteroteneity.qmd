...
* Imagine the pixe value is formed by the addition of the return from $m$ elementary backscatterers:
$$
...
knitr::include_graphics("/Users/acfrery/Library/CloudStorage/OneDrive-VictoriaUniversityofWellington-STAFF/Documents/Slides/entropy.jpg")
```

## Why do we use the entropy?

According to @EnergyandInformation, Shannon used this name because
...
This means that
\begin{equation*}
  \color{DarkOrchid}{T_\lambda(\mathcal{G}^0_I)
  =
...
* Check if it close to the theoretical Tsallis entropy under the $\Gamma_{\mathrm{SAR}}$ model or to the Tsallis entropy under the $\Gamma_{\mathrm{SAR}}(\mu,L)$.
:::
...
## Direct estimation of the entropy
...
@vasicek1976test and @Ebrahimi1994 proposed spacing-based estimation for the Shannon entropy.
@AlLabadi2024 followed the approach for the Rényi entropy.
We proceed in the same way, and obtain a non-parametric estimator for the Tsallis entropy.
...
## But we want [more]{.alert}

We improve $\widehat T_{\lambda}(\mathbf{Z})$ by simple bootstrap, and use $\widetilde T_{\lambda}(\mathbf{Z})$ that

* is less biased than $\widehat T_{\lambda}(\mathbf{Z})$, and
* is more sensitive to outliers (recall that we will measure departures from the fully-developed speckle hypothesis).
