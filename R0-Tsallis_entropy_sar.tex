...
framework that generates per-pixel \(p\)-value maps for detecting
departures from fully-developed speckle. Experimental validation using
both simulated data and SAR imagery confirm the method's precision in
detecting texture variability and discriminating between homogeneous and
heterogeneous regions. The proposed approach offers a principled,
interpretable tool for automated SAR image analysis without requiring
training data or scene-specific calibration.
\end{abstract}
...
The Tsallis entropy~\citeproc{ref-Tsallis1988}{{[}20{]}} is a
...
\(\mu\)~\citeproc{ref-Nascimento2024}{{[}24{]}}. Expressing \(\gamma\)
...
relationship between \(\mathcal{G}^0_I\) and gamma laws based on Tsallis
entropy. It is proven in the \href{}{Appendix}.
...
Ref.~\citeproc{ref-Crzcgorzewski1999}{{[}29{]}}.
...
(physically accepted) is illustrative: the estimator performs properly
for \(L\geq1\). The results confirm that, for \(\lambda=0.85\), the
bootstrap-corrected estimator \(\widetilde{T}_{\lambda}\) with \(B=200\)
reduces both bias and MSE, compared with the original estimator
\(\widehat{T}_{\lambda}\), with significant improvements for small
samples, as shown in Figure~\ref{fig-bias_mse_Tsallis}.
...
al.~\citeproc{ref-Gomez2017}{{[}33{]}} as a tool to assess the
