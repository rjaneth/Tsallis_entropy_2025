...
distribution, heterogeneity, SAR, hypothesis testing},
  colorlinks=true,
  linkcolor={black},
  filecolor={Maroon},
  citecolor={black},
...
\href{}{Appendix}. Equation~\eqref{eq:GI0-Tsallis} can be interpreted
as: \begin{equation*}
...
\(5\times5\) to \(11\times11\) (\emph{i.e.}, \(N_{\min}=2\),
\(N_{\max}=5\), hence \(W_{\min}=5\) and \(W_{\max}=11\)). For
comparison, a fixed \(7\times7\) window was also considered. For each
window, \(B=100\) bootstrap replications were performed to estimate the
...
moderate computational cost. Stable results can already be achieved with
...
windowing scheme to balance speckle suppression and texture
...
well calibrated, robust to small samples sizes, and effective in
delineating heterogeneous regions from homogeneous ones. A Monte Carlo
validation confirmed that the test maintains proper control of the
Type~I error while improving detection power with increasing sample
size.
...
executed in \texttt{Quarto} and is fully reproducible. We used
\texttt{RStudio} version 2025.09.1+401 and \texttt{R} version 4.5.1.
...
Core\texttrademark{} Ultra 9 285 CPU (2.5 GHz, 24 cores), 128 GB RAM,
and an NVIDIA\textsuperscript{\textregistered} GeForce RTX 5090 GPU (31
GB VRAM), running Windows 11 Pro (24H2).
