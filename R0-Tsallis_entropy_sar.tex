...
yields an unsupervised and interpretable statistical learning approach
that produces \(p\)-value maps, providing per-pixel significance levels
for detecting departures from fully-developed speckle. Experimental
...
when~\(m,n\to\infty\) and \(m/n\to 0\). We use the heuristic formula
\(m = \lfloor \sqrt{n} + 0.5 \rfloor\) proposed
in~\citeproc{ref-Crzcgorzewski1999}{{[}29{]}}.
...
\citeproc{ref-Alpala2024}{{[}30{]}}, we refine the nonparametric entropy
...
estimators~\citeproc{ref-vasicek1976test}{{[}26{]}},
\citeproc{ref-VanEs1992}{{[}31{]}}, for sufficiently large samples,
...
Delta Method
(see~\citeproc{ref-MathematicalStatisticsvII2015}{{[}32{]}}, Ch. 7), its
...
statistic may overlap areas with different distribution. Intuitively,
...
by Gomez et al.~\citeproc{ref-Gomez2017}{{[}33{]}} as a tool to assess
...
zones~\citeproc{ref-Frery2019a}{{[}34{]}}. As the \(\alpha\) value
...
and water bodies~\citeproc{ref-Neto2023}{{[}35{]}}.
...
results for \(\eta=1.0\), \(2.0\), \(3.0\), and \(4.0\). The
corresponding binary decision maps at a \(0.05\) significance level are
shown in Figures~\ref{fig:simbc1}--\ref{fig:simbc5}. We used a
significance level of \(0.05\) because it is a standard and practical
threshold: \(p\)-values below this value provide sufficient evidence to
reject local homogeneity without generating an excessive number of false
detections.
...
(\(W_{\max}=11\)), for smoothing parameters \(\eta=1.0\), \(2.0\),
\(3.0\), and \(4.0\). In all cases, \(B=100\) bootstrap replications
were performed per window.
...
where speckle is strongest. The adaptive map with \(\eta = 2.0\)
provides the clearest separation between urban blocks, water, and
intermediate textured areas. For larger values, such as \(\eta = 3.0\)
and \(\eta = 4.0\), the maps become increasingly saturated in red
because the window grows more often and tends to mix classes within the
local window. This produces thicker building fronts and more detections
in moderately textured zones, but with a visible loss of spatial
precision.
...
inspection suggests that \(\eta\) values between \(2.0\) and \(3.0\)
provide the most balanced results. With \(\eta = 1.0\), the outcome is
close to the fixed-window case, offering little improvement, whereas
\(\eta = 4.0\) tends to oversmooth the images, causing blurred edges and
loss of fine details (see panels (j) in
Figures~\ref{fig:munich}--\ref{fig:foggia}).
