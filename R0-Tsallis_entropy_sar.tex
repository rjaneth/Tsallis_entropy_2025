...
small sample sizes, the estimator is refined through a bootstrap
correction. In addition, we integrate the test within an adaptive
windowing strategy which locally selects the optimal window size: larger
in homogeneous regions to stabilize estimation, and smaller in
heterogeneous areas to preserve structural details. This combination
yields an unsupervised and interpretable statistical learning that
produces \(p\)-value maps highlighting heterogeneous areas. Experimental
results from both simulated data and actual SAR images demonstrate the
method precision in detecting texture variability and distinguishing
...
Tsallis entropy with its theoretical value under the \(\mathcal{G}^0_I\)
and Gamma probabilistic assumptions. To improve performances, we
incorporate a bootstrap procedure that reduces bias in small samples. In
addition, we embed the test within an adaptive windowing framework,
inspired by the method of Park et al.~\citeproc{ref-Park1999}{{[}21{]}}.
Instead of relying on fixed-size sliding windows (e.g., \(7 \times 7\)),
the adaptive procedure adjusts the window size locally: it enlarges in
homogeneous regions to improve statistical precision, and shrinks in
...
generates interpretable \(p\)-value maps, making it suitable for
...
literature~\citeproc{ref-Nascimento2024}{{[}24{]}} the texture parameter
...
to the Shannon entropy, making the Tsallis measure a one-parameter
...
\label{eq:GI0-Tsallis}
  ...
  \bigr]\Bigr\},
\end{multline} where \(T_\lambda(\mathcal{D}) = T_\lambda(Z)\) with
\(Z \sim \mathcal{D}\). From now on, we use this equality to highlight
the relationship between \(\mathcal{G}^0_I\) and gamma laws based on
Tsallis entropy. The derivation of the latter result is proven in the
\href{}{Appendix}.
...
\end{equation} Let \(\bm{Z}=(Z_1,Z_2,\dots,Z_n)\) be a \(n\)-points
...
\(m\in\{1,2,\dots,\lfloor (n-1)/2\rfloor\}\), set the \(m\)-spacing \[
...
estimator~\citeproc{ref-vasicek1976test}{{[}26{]}}. While a formal proof
of consistency is not provided here, the estimator follows the same
\(m\)-spacing construction used for the Shannon
entropy~\citeproc{ref-vasicek1976test}{{[}26{]}},
\citeproc{ref-Ebrahimi1994}{{[}27{]}} and for the Rényi
entropy~\citeproc{ref-AlLabadi2024}{{[}28{]}}. Empirically,
\(\widehat T_\lambda\) exhibits the expected large-sample behavior
when~\(m,n\to\infty\) and \(m/n\to 0\). We use the heuristic
...
For the particular case \(L=1\), we found that the optimal value is
\(\lambda=1.2\), which offers the best MSE--bias trade-off.
...
\(Z \sim \Gamma_{\mathrm{SAR}}(1,5)\). The choice of \(L=5\) and
\(\mu=1\) (physically accepted) is illustrative: the estimator performs
properly for \(L\geq1\). The results confirm that, for \(\lambda=0.85\),
the bootstrap-corrected estimator \(\widetilde{T}_{\lambda}\) with
\(B=200\) reduces both bias and MSE, compared with the original
estimator \(\widehat{T}_{\lambda}\), with significant improvements for
small samples, as shown in Figure~\ref{fig-bias_mse_Tsallis}.
...
\((Z\sim\Gamma_{\mathrm{SAR}}(\mu, L))\) or a heterogeneous
\((Z\sim\mathcal{G}^0_I(\alpha, \mu, L))\) region, as follows:
...
number of looks \(L\geq1\) is known, we define the test statistics under
...
fitted value under homogeneity. Values close to zero indicate that the
...
Delta Method
(see~\citeproc{ref-MathematicalStatisticsvII2015}{{[}31{]}}, Ch. 7), its
...
test size (or false alarm) and test power (detection probability). These
provide insight into the probability of incorrect decisions in
hypothesis testing.
...
by Gomez et al.~\citeproc{ref-Gomez2017}{{[}32{]}} as a tool to assess
...
zones~\citeproc{ref-Frery2019a}{{[}33{]}}. As the \(\alpha\) value
...
and water bodies~\citeproc{ref-Neto2023}{{[}34{]}}.
...
\multirow{-4}{*}{\raggedright\arraybackslash Simulated} & Adapt. $\eta=3$ & \bm{$0.983$} & \bm{$0.966$} & \bm{$0.983$}\\
\cmidrule{1-5}
...
\multirow{-4}{*}{\raggedright\arraybackslash San Francisco} & Adapt. $\eta=3$ & $0.841$ & $0.729$ & $0.866$\\
\cmidrule{1-5}
...
\multirow{-4}{*}{\raggedright\arraybackslash Munich} & Adapt. $\eta=3$ & \bm{$0.935$} & \bm{$0.880$} & \bm{$0.940$}\\
\cmidrule{1-5}
...
\multirow{-4}{*}{\raggedright\arraybackslash Dublin} & Adapt. $\eta=3$ & \bm{$0.901$} & \bm{$0.844$} & \bm{$0.926$}\\
\cmidrule{1-5}
...
\multirow{-4}{*}{\raggedright\arraybackslash Foggia} & Adapt. $\eta=3$ & \bm{$0.951$} & \bm{$0.907$} & \bm{$0.953$}\\
\bottomrule
\end{tabular}}
