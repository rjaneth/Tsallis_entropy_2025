...
assumption. To improve performance, we incorporate a bootstrap procedure
...
et al.~\citeproc{ref-Park1999}{{[}21{]}}. Instead of relying on
...
literature~\citeproc{ref-Nascimento2010}{{[}22{]}} the texture parameter
...
\citeproc{ref-Frery1997}{{[}12{]}}. Specifically, for fixed \(\mu\), \[
...
Charvát~\citeproc{ref-Havrda1967}{{[}23{]}} in the context of
...
support of \(f(z)\). When \(\lambda \to 1\), \(T_\lambda(Z) \to H(Z)\),
...
\(L \in \{5,18\}\). In both selected cases, the dashed curves of
...
Vasicek~\citeproc{ref-vasicek1976test}{{[}24{]}} and Ebrahimi et
al.~\citeproc{ref-Ebrahimi1994}{{[}25{]}} for Shannon entropy, and the
methodology for Rényi entropy proposed by Al-Labadi et
al.~\citeproc{ref-AlLabadi2024}{{[}26{]}}, we derive a nonparametric
...
\(m\in\{1,\dots,\lfloor (n-1)/2\rfloor\}\), the \(m\)-spacing is defined
by \[
...
estimator~\citeproc{ref-vasicek1976test}{{[}24{]}}. This estimator is
...
that \(\lambda=0.85\) yields the lowest MSE while maintaining a nearly
zero bias, thus providing a favorable trade-off between bias and
variance.
...
\citeproc{ref-Alpala2024}{{[}27{]}}, we refine the nonparametric entropy
...
entropy estimators of \(\Gamma_{\mathrm{SAR}}\).}
...
\(T_{\lambda}(\Gamma_{\mathrm{SAR}})\). Instead, significant deviations
...
need to estimate \(\mathcal{G}^0_I\) parameters such as \(\alpha\),
...
around zero, confirming that under \(\mathcal{H}_0\), the test has mean
approximately \(0\); at the same time, their moderately heavy tails
reveal sensitivity to departures from homogeneity, which is desirable
for detecting subtle textures.
...
\citeproc{ref-vasicek1976test}{{[}24{]}},
\citeproc{ref-VanEs1992}{{[}28{]}}, for sufficiently large samples,
...
\(S_{\widetilde{T}}(\bm{Z}; L)\) for the observed data; then,
...
the \(p\)-value with the threshold \(0.05\). The corresponding workflow
is illustrated in Figure~\ref{fig:entropy-diagram-test}.
...
scene. To this aim, for each pixel \((i,j)\), let \(w_{ij}\) be the
window centered at \((i,j)\) with radius \(N_{ij}\), defined as
\begin{equation}
...
by Gomez et al.~\citeproc{ref-Gomez2017}{{[}29{]}} as a tool to assess
...
zones~\citeproc{ref-Frery2019a}{{[}30{]}}. As the \(\alpha\) value
decreases, it indicates regions with moderate texture (in the
\(\left[-6,-3\right]\) region), related to forest zones, while values
below \(-6\) correspond to textureless regions, such as pasture,
agricultural fields, and water bodies~\citeproc{ref-Neto2023}{{[}31{]}}.
...
with a size ranging from \(5\times5\) (\(N_{\min}=5\)) to \(11\times11\)
...
progression: orange (\(p\!\approx\!0.2\)--\(0.4\)) denotes transitional
texture, whereas yellow--green to cyan (\(p\!\approx\!0.4\)--\(0.8\))
...
quantitative metrics (\(F_1\), \(\kappa\), OA) showed that the adaptive
Tsallis-based configuration outperforms the fixed sliding window in all
evaluated scenes. The method thus provides a reliable and interpretable
tool for heterogeneity detection in SAR imagery.
...
S. Sant'Anna, {``A model for extremely heterogeneous clutter,''}
