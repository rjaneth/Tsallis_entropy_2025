...
Our approach builds on the adaptive windowing algorithm for SAR images proposed by Park et al. [@Park1999], which adjusts the analysis window size according to the local homogeneity of the scene. For each pixel $(i,j)$, let $w_{ij}$ be the windows centered at the pixel $(i,j)$ of radius $N_{ij}$ defined as
\begin{equation}
w_{ij} = \{(i',j') / i-N_{ij} \le i' \le i+N_{ij} \, \wedge \, j-N_{ij} \le j' \le j+N_{ij}\}.
\end{equation}
Then, the border of this window is given by
\begin{multline}
b_{ij} = \{(i',j') \in w_{ij}/ i' = i-N_{ij} \, \vee \, i' = i+N_{ij} \, \vee \\ j' = j-N_{ij} \, \vee \, j' = j+N_{ij}\}.
\end{multline}
The coefficient of variation is computed over the border of the current window, as follows:
\begin{equation}
C_{ij} = \frac{\sigma_{ij}}{\mu_{ij}},
\end{equation}
where the mean $\mu_{ij}$ and the standar deviation $\sigma_{ij}$ are calculated using only the border pixels $b_{ij}$ of the window $w_{ij}$. 
In homogeneous areas, 
\begin{equation}
C_{ij} \approx \sigma_n := \frac{0.523}{\sqrt{L}}.
\end{equation}
...
\end{cases}
$$
where $N_{\max}$ and $N_{\min}$ denote the maximum and minimum allowed window size, respectively. 
...
Once the window radius $N_{ij}$ has been determined for the pixel $(i,j)$, the corresponding full window is extracted and the proposed test in \eqref{eq-test-tsallis} is applied to obtain the local test statistic. This produces two outputs: 
...
Each region is filled with data simulated from the $\mathcal{G}^0_I$ distribution, using the expression&nbsp;\eqref{E:gi02}, considering different combinations of the texture parameter $\alpha$ and the mean $\mu$, as indicated by the labels in each quadrant of the image.
...
zones&nbsp;[@Frery2019a]. 
As the value decreases, it indicates regions
...
The proposed Tsallis-based test statistic, $S_{\widetilde{T}_\lambda}$, was applied to the simulated image using the adaptive windowing strategy with side lengths ranging from $5\times 5$ ($N_{\min}=5$) to $11\times 11$ ($N_{\max}=11$), with the smoothing parameter set to $\eta = 5$. Figure&nbsp;\ref{fig:pvalueL9} displays the resulting $p$-value map for $L=9$. 
...
\begin{figure*}[hbt]
   ...
   \hspace{0.000001\textwidth}
    \begin{subfigure}{0.34\textwidth}
        ...
        \caption{Simulated image with $L=9$}
   ...
   \hspace{0.000001\textwidth}
    \begin{subfigure}{0.31\textwidth}
        ...
        \label{fig:pvalueL9}
    \end{subfigure}
    \caption{Results on simulated data with $S_{\widetilde{T}_{\lambda}}$, using adaptive windows from $N_{\min}=5$ to $N_{\max}=11$.}
...
The selected SAR scenes were acquired in HH polarization for the outskirts of Munich (Germany) and Dublin Port (Ireland), and in VV polarization for Foggia (Italy), as shown in Figures.&nbsp;\ref{fig:munich-sar}–\ref{fig:foggia-sar}. 
The corresponding optical images (Figures.&nbsp;\ref{fig:munich-o}–\ref{fig:foggia-o}) illustrate the land cover context.
...
SAR_data <- data.frame(
  Site = c("Munich", "Dublin", "Foggia" ),
  Mission = c("UAVSAR", "TanDEM-X", "Iceye"),
  Band = c("L", "X", "X"),
  Size = c( "$1024\\times1024$", "$1100\\times1100$", "$1900\\times1900$"),
  L = c( 12, 16, 24),
  Resol = c( "$4.9\\times7.2$", "$1.35\\times1.35$", "$2.08\\times2.08$"),
  Date = c("16-04-2015", "03-09-2017", "28-03-2024")
)
...
```
We applied the proposed test statistic $S_{\widetilde{T}_{\lambda}}$ to real SAR data using adaptive local sliding windows ranging from $N_{\min}=5$ to $N_{\max}=11$, with the smoothing parameter set to $\eta = 5$. The results are illustrated in Figures&nbsp;\ref{fig:munichp}–\ref{fig:foggiap}, which display the original SAR image, the corresponding optical reference, and the resulting $p$-value maps.
```{=latex}
...
\begin{figure*}[!t]
    ...
    \begin{subfigure}{0.256\textwidth}
   ...
   \hspace{0.01\textwidth}
    \begin{subfigure}{0.34\textwidth}
        \includegraphics[width=\linewidth]{./Figures/munich_1024_085_eta5.png}
        \caption{$p$-value map   }
        \label{fig:munichp}
    \end{subfigure}
    \caption{Results on SAR data over Munich, Germany, using the test statistic $S_{\widetilde{T}_{\lambda}}$ with adaptive windows from $N_{\min}=5$ to $N_{\max}=11$.}
    \label{fig:munich}
\end{figure*}
```
...
\begin{figure*}[!t]
   ...
   \hspace{0.01\textwidth}
    \begin{subfigure}{0.335\textwidth}
        ...
        \label{fig:dublinp}
    \end{subfigure}
   \caption{Results on SAR data over Dublin, Ireland, using the test statistic $S_{\widetilde{T}_{\lambda}}$ with adaptive windows from $N_{\min}=5$ to $N_{\max}=11$.}
    \label{fig:dublin}
\end{figure*}
```
...
\begin{figure*}[!t]
   ...
   \hspace{0.01\textwidth}
    \begin{subfigure}{0.33\textwidth}
        \includegraphics[width=\linewidth]{./Figures/tsallis_Adp_foggia_085_L24_eta5.png}
        \caption{$p$-value map   }
        \label{fig:foggiap}
    \end{subfigure}
   \caption{Results on SAR data over Foggia, Italy, using the test statistic $S_{\widetilde{T}_{\lambda}}$ with adaptive windows from $N_{\min}=5$ to $N_{\max}=11$.}
    \label{fig:foggia}
\end{figure*}
```

The $p$-value maps provide a clear separation between homogeneous and heterogeneous areas. Dark-red tones ($p < 0.05$) cluster around highly textured features, including urban blocks, industrial and harbor facilities, and road networks. 
These regions exhibit strong departures from homogeneity, consistent with the expected scattering variability in built environments. Intermediate tones (green and yellow, $0.2 < p < 0.6$) appear in semi-structured areas such as agricultural fields and mixed land covers, reflecting moderate texture. 
By contrast, blue tones ($p \approx 1$) dominate in smooth regions such as open water bodies, uniform pastures, and large homogeneous crop zones, where the data behave in accordance with the null hypothesis.

The results follow from the adaptive windowing: larger windows are selected in homogeneous regions to enhance stability and suppress noise, whereas smaller windows are used in heterogeneous areas to preserve structural details and prevent class mixing. 
This balance allows the method to preserve edges and delineate boundaries in heterogeneous zones, while still providing robust detection of broader homogeneous areas.
