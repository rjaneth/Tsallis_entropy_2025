...
[S]{.IEEEPARstart}[ynthetic]{}
 Aperture Radar  (SAR) technology has become a key tool in remote sensing, offering high-resolution imaging capabilities independent of solar illumination and weather conditions, which enables continuous and reliable Earth observation&nbsp;[@Mondini2021]. 
 SAR images are widely applied in environmental monitoring&nbsp;[@Amitrano2021], 
 disaster assessment&nbsp;[@Ge2020], 
 agriculture&nbsp;[@Changan2019], 
 urban planning&nbsp;[@Esch2010;@NovelTechniquesforBuiltupAreaExtractionfromPolarimetricSARImages2019], and 
 maritime surveillance&nbsp;[@Cao2024;@OntheDetectionandLongTermPathVisualisationofa68Iceberg], among other fields&nbsp;[@Moreira2013].
...
Among the statistical models used for SAR intensities, the $\mathcal{G}_I^0$ distribution &nbsp;[@Frery1997; @Ferreira2020] is widely recognized for its ability to capture different levels of scene heterogeneity. 
...
In this work, we propose a statistical test based on a nonparametric estimator of the Tsallis entropy to distinguish between homogeneous and heterogeneous regions in SAR data. The test compares the estimated Tsallis entropy with its theoretical value under the Gamma model assumption. To improve performance, we incorporate a bootstrap procedure that reduces bias in small samples. In addition, we embed the test within an adaptive windowing framework, inspired by the method of Park et al.&nbsp;[@Park1999]. Instead of relying on fixed-size sliding windows (e.g., $7 \times 7$), the adaptive procedure adjusts the window size locally: it enlarges in homogeneous regions to improve statistical stability, and shrinks in heterogeneous regions to avoid mixing different structures and to preserve local details.
...
In the analysis of SAR imagery, where speckle is inherent, statistical modeling is essential.
For SAR intensity data, two main probability models are employed: the Gamma SAR distribution ($\Gamma_{\mathrm{SAR}}$), a reparametrization of the classical Gamma law suited to fully developed speckle, and the $\mathcal{G}^0_I$ distribution, which describes different levels of heterogeneity&nbsp;[@Frery1997].
...
\end{equation}
Similarly, $Z \sim \mathcal{G}^0_I(\alpha,\gamma,L)$ has density:
\begin{align}
...
\end{align}
Here, $\mu > 0$ is the mean intensity, $\gamma > 0$ is a scale parameter, $\alpha < 0$ is the texture (heterogeneity) parameter of the $\mathcal{G}^0_I$ distribution, $L \geq 1$ is the number of looks (either nominal or estimated, thus not restricted to integer values), $\Gamma(\cdot)$ is
the gamma function, and $\mathbbm 1_{A}(\cdot)$ is the indicator function of the set $A$.
...
\end{multline}
The $\Gamma_{\mathrm{SAR}}$ model is a particular case of the $\mathcal{G}^0_I$ distribution [@Frery1997]. Specifically, for fixed $\mu$,
$$
...
When $\lambda \to 1$, $T_\lambda(Z) \to H(Z)$, making Tsallis a one-parameter generalization of Shannon entropy.
...
As $\alpha \to -\infty$ (fully developed speckle), $\Delta_\alpha \to 0$ and \eqref{eq:GI0-Tsallis} reduces to the homogeneous case in \eqref{eq:gammasar-Tsallis}, \emph{i.e.},
$$
\lim_{\alpha \to -\infty} T_{\lambda}\bigl(\mathcal{G}^0_I(\mu, \alpha, L)\bigr) =  T_{\lambda}\bigl(\Gamma_{\mathrm{SAR}}(\mu, L)\bigr).
$$ 

@fig-tsallis-convergence_L18 illustrates the convergence of the Tsallis entropy for different values of $\alpha$ and $L \in \{5,18\}$. In both selected cases, the dashed curves of $T_\lambda(\mathcal{G}^0_I)$ approach the solid black curve of $T_\lambda(\Gamma_{\mathrm{SAR}})$, confirming the limiting behavior.
...
\end{equation}
Let $\bm{Z}=(Z_1,Z_2,\dots,Z_n)$ be an i.i.d.\ sample from $F$, with order statistics $Z_{(1)}\le Z_{(2)} \le \cdots\le Z_{(n)}$. For a window size $m\in\{1,\dots,\lfloor (n-1)/2\rfloor\}$, the $m$-spacing is defined by
$$
...
We investigate the optimal order $\lambda$ for the Tsallis entropy estimator using simulated samples of size $n = 49$ drawn from $Z \sim \Gamma_{\mathrm{SAR}}(1,5)$. 
...
```{r fig-optimal_order-tsallis,  echo=FALSE, message=FALSE, warning=FALSE,  out.width=".9\\linewidth",  fig.pos="hbt", fig.cap="Bias and MSE as a function of $\\lambda$, for the Tsallis entropy estimator, with $n = 49$ and $L = 5$."}
#fig.show="hide", 
# eval=FALSE,  # 
...
A Monte Carlo study with $1000$ replicas for each sample size $n \in \{9, 25, 49, 81, 121\}$ was conducted using samples from $\Gamma_{\mathrm{SAR}}(1,5)$. The choice of $L=5$ and $\mu=1$ is illustrative: the estimator performs properly for $L>1$, while for $L=1$ the bootstrap correction produces no noticeable improvement. The results confirm that, for $\lambda=0.85$, the bootstrap-corrected estimator $\widetilde{T}_{\lambda}$ with $B=200$ reduces both bias and MSE compared to the original estimator $\widehat{T}_{\lambda}$, with significant improvements for small samples, as shown in Figure&nbsp;\ref{fig-bias_mse_Tsallis}. 
...
```{r fig-bias_mse_Tsallis,  echo=FALSE, message=FALSE, warning=FALSE,  out.width=".9\\linewidth",  fig.pos="hbt",  fig.cap="Bias and MSE for the Tsallis entropy estimators of $\\Gamma_{\\mathrm{SAR}}$.", fig.asp=0.75}
...
We test whether the observed data come from a homogeneous ($\Gamma_{\mathrm{SAR}}$) or a heterogeneous ($\mathcal{G}^0_I$) region, as follows:
...
\end{cases}
\end{equation}
Under $\mathcal{H}_0$, the expected value of the entropy estimator should match the theoretical $T_{\lambda}(\Gamma_{\mathrm{SAR}})$. 
Instead, significant deviations indicate heterogeneity.

## The Proposed Test

We propose a test statistic that identifies the discrepancy between estimated and theoretical entropy under homogeneity. Since the number of looks $L\geq1$ is known, we define the test as follows:
\begin{multline}
...
$$
\emph{i.e.}, the difference between the estimated entropy and the expected value under homogeneity. Values close to zero indicate that the region behaves like fully developed speckle, while large positive values signal entropy excess, hence heterogeneity.
This formulation avoids the need to estimate $\mathcal{G}^0_I$ parameters  such as $\alpha$, offering a simple, interpretable, and statistically grounded test. Moreover, the proposed method remains effective for small samples, aided by the bootstrap bias correction.

Figure&nbsp;\ref{fig-densities-tsallis6} shows the empirical distribution of $S_{\widetilde{T}_{\lambda}}$ under $\mathcal{H}_0$, obtained from $10^4$ Monte Carlo simulations with different sample sizes $n \in \{25, 49, 81,121\}$, $\lambda = 0.85$, and $L \in \{5,18\}$. The empirical densities are tightly concentrated around zero, confirming that under $\mathcal{H}_0$, the test  has mean approximately $0$; at the same time, their moderately heavy tails reveal sensitivity to departures from homogeneity, which is desirable for detecting subtle textures.
...
\end{equation*} 
is asymptotically standard Normal distributed for large $n$. Consequently, two-sided $p$-values are obtained as $2\Phi(-|\varepsilon|)$, where $\Phi(\cdot)$ is the cumulative distribution function of the standard Normal distribution.

The practical procedure is as follows: first, compute the test statistic $S_{\widetilde{T}}(\bm{Z}; L)$ for the observed data; then, standardize it using the estimated mean $\widehat{\mu}_S$ and the standard deviation $\widehat{\sigma}_S$; next, compute the $p$-value as $2\Phi(-|\varepsilon|)$; and finally, make a decision by comparing the $p$-value with the threshold $0.05$. 
The corresponding workflow is illustrated in Figure&nbsp;\ref{fig:entropy-diagram-test}.
...
The size of a statistical test, also known as the Type&nbsp;I&nbsp;error rate, refers to the probability of incorrectly rejecting the null hypothesis $\mathcal{H}_0$ when it is in fact true. In hypothesis testing, practitioners typically specify a nominal significance level, commonly set at \SI{1}{\percent}, \SI{5}{\percent}, or \SI{10}{\percent}, which represent acceptable probabilities of committing a Type&nbsp;I error. 
A well-calibrated test should reproduce these levels empirically across sample sizes and conditions.
To assess this, we performed $1000$ Monte Carlo replications under $\mathcal{H}_0$ with data from a $\Gamma_{\mathrm{SAR}}$ distribution with $\mu=1$, computing the test statistic via a bootstrap entropy estimator using $B=100$ and $\lambda=0.85$. 
...
The power of a test is its probability of correctly rejecting $\mathcal{H}_0$ when it is false. Equivalently, it equals $1-\beta$, where $\beta$ is the Type&nbsp;II error rate. 
High power thus indicates strong sensitivity to departures from $\mathcal{H}_0$. To assess power, we simulated data under the alternative hypothesis $\mathcal{H}_1$, assuming a $\mathcal{G}^0_I$ distribution with $\mu=1$ and $\alpha=-2$. 
...
Our approach builds on the adaptive windowing algorithm for SAR images proposed by Park et al.&nbsp;[@Park1999], which adjusts the analysis window size according to the local homogeneity of the scene. To this aim, for each pixel $(i,j)$, let $w_{ij}$ be the window centered at $(i,j)$ with radius $N_{ij}$, defined as
\begin{equation}
w_{ij} = \{(i',j') \, / \, i-N_{ij} \le i' \le i+N_{ij} \,\wedge\, j-N_{ij} \le j' \le j+N_{ij}\}.
\end{equation}
Then, the border of this window is
\begin{multline}
...
The homogeneity criterion is defined by comparing $C_{ij}$ with an adaptive threshold $U_{ij}$:
\begin{equation}
...
Once the window radius $N_{ij}$ has been determined for pixel $(i,j)$, the corresponding full window is extracted and the proposed test in \eqref{eq-test-tsallis} is applied to obtain the local test statistic. 
This produces two outputs: (i) a map of test statistics over the image, and (ii) a window-size map $W_{ij}$ that records the locally selected window side (larger in homogeneous regions, smaller in heterogeneous ones). 
...
Figure&nbsp;\ref{fig:sim_scene} shows the simulated image of size $500\times500$ pixels, generated from the structural design proposed by Gomez et al.&nbsp;[@Gomez2017] as a tool to assess the performance of speckle-reduction filters. 
...
interpreting texture characteristics. Values belonging to $]-3, 0[$ suggest extremely textured targets, such as urban zones&nbsp;[@Frery2019a]. 
As the $\alpha$ value decreases, it indicates regions with moderate texture (in the $\left[-6,-3\right]$ region), related to forest zones, while values below $-6$ correspond to textureless regions, such as pasture, agricultural fields, and water bodies&nbsp;[@Neto2023].

The proposed Tsallis-based test statistic, $S_{\widetilde{T}\lambda}$, was applied to the simulated image using the adaptive windowing strategy with a size ranging from $5\times5$ ($N_{\min}=5$) to $11\times11$ ($N_{\max}=11$). 
For comparison, a fixed $7\times7$ window was also considered. 

Figure&nbsp;\ref{fig:simc1} shows the $p$-value map with the fixed window, while Figures&nbsp;\ref{fig:simc2}-\ref{fig:simc4} display the adaptive results for $\eta=1.0$, $2.0$, and $3.0$. The corresponding binary decision maps at a $0.05$ significance level are shown in Figures&nbsp;\ref{fig:simbc1}-\ref{fig:simbc4}.
...
Intermediate colors follow a smooth progression: orange ($p\!\approx\!0.2$–$0.4$) denotes transitional texture, whereas yellow–green to cyan ($p\!\approx\!0.4$–$0.8$) marks increasingly near-homogeneous areas.
...
Visual inspection suggests that $\eta$ values between $2.0$ and $3.0$ provide the most balanced results, as larger values tend to blur the edges.
...
We evaluated the proposed test statistic on four SAR scenes from different radar missions, covering San Francisco (USA), the outskirts of Munich (Germany), Dublin Port (Ireland), and the area around Foggia (Italy). The first three SAR images were acquired in HH polarization and the fourth in VV polarization (see Figures.&nbsp;\ref{fig:sar1}, \ref{fig:sar2}, \ref{fig:sar3} and \ref{fig:sar4}).
The corresponding optical images (Figures.&nbsp;\ref{fig:opt1}, \ref{fig:opt2}, \ref{fig:opt3} and \ref{fig:opt4}) illustrate the land cover context.
```{=latex}
...
product of azimuth and range looks provided in the metadata of SNAP (Sentinel Application Platform) toolbox. We
...
The proposed Tsallis-based test statistic, $S_{\widetilde{T}{\lambda}}$, was applied to four SAR intensity images. 
For each scene, results were obtained using a fixed $7 \times 7$ sliding window and the adaptive windowing scheme with side lengths ranging from $5\times5$ ($N_{\min}=5$) to $11\times11$ ($N_{\max}=11$), for smoothing parameters $\eta=1.0$, $2.0$, and $3.0$.
...
In the $p$-value heatmaps, dark-red tones ($p \approx 0$) concentrate on heterogeneous areas such as urban blocks, harbor structures, and field boundaries, while blue tones ($p \approx 1$) dominate open water and uniform surfaces. 
...
The binary maps make the separation between homogeneous and heterogeneous areas explicit, showing that the adaptive strategy preserves fine details that are partially lost with the fixed window.
...
Visual inspection suggests that $\eta=2.0$ and $\eta=3.0$ yield the most balanced results. 
With $\eta=1.0$, the outcome is close to the fixed-window case, offering little improvement. 
Values above $\eta=3.0$ tend to oversmooth the images, causing blurred edges and loss of fine details. 
...
For the four real SAR datasets (San Francisco, Munich, Dublin, and Foggia), eight polygonal ROIs were selected per scene (four per class), also labeled as class 1 for heterogeneous and class 0 for homogeneous, as shown in Figure&nbsp;\ref{fig:image_roi}. 
All polygons were rasterized to the spatial grid of each SAR image, producing a consistent binary reference mask used for evaluation.
...
The quantitative evaluation considered the fixed $7 \times 7$ sliding window and the adaptive window controlled by the parameter $\eta$. 
The results are shown in @tbl-metrics-tsallis1.
...
In these cases, the larger adaptive window was more effective at capturing the spatial structure of homogeneous areas while maintaining class separation.
