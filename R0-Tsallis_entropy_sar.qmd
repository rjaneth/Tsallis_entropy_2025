...
We aim to determine the optimal order $\lambda$ for the Tsallis entropy estimator using simulated samples of size $n = 49$ from  $Z\sim \Gamma_{\text{SAR}}(1,5)$.
...
```{r fig-optimal_order-tsallis,  echo=FALSE, message=FALSE, warning=FALSE,  out.width="50%",  fig.pos="H", fig.cap="Bias and MSE as a function of $\\lambda$, for the Tsallis entropy estimator, with $n = 49$ and $L = 5$.", fig.width=5, fig.height=3}
#fig.show="hide", 
# eval=FALSE,  # 
...
```{r fig-bias_mse_Tsallis,  echo=FALSE, message=FALSE, warning=FALSE,  out.width="50%",  fig.pos="H",  fig.cap="Bias and MSE for the Tsallis entropy estimators of $\\Gamma_{\\text{SAR}}$, with $L=5$", fig.width=5.5, fig.height=4}
...
We propose a statistic test that identifies the discrepancy between estimated and theoretical entropy under homogeneity. Since $L\geq1$ is known, we define the test statistic as follows:
\begin{multline}
...
```{r fig-densities-tsallis4,  echo=FALSE, message=FALSE, warning=FALSE,  out.width="50%", fig.pos="H",  fig.cap="Empirical densities of $S_{\\widetilde{T}_{\\lambda}}(\\bm{Z}; L)$ under $\\mathcal{H}_0$, with $\\lambda=0.85$.", fig.width=9, fig.height=4.5}
#fig.show="hide", 
# eval=FALSE,  #
