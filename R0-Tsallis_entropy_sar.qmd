...
```{r fig-densities-tsallis6,  echo=FALSE, message=FALSE, warning=FALSE,  out.width=".9\\linewidth", fig.pos="H",  fig.cap="Empirical densities of $S_{\\widetilde{T}_{\\lambda}}(\\bm{Z}; L)$ under $\\mathcal{H}_0$, with $\\lambda=0.85$."}
#fig.show="hide", 
...
To assess this, we performed $1000$ Monte Carlo replications under $\mathcal{H}_0$ with data from a $\Gamma_{\mathrm{SAR}}$ distribution ($\mu=1$), computing the test statistic via a bootstrap entropy estimator ($B=100$, $\lambda=0.85$). The empirical size was then estimated as the proportion of replications in which the null hypothesis was incorrectly rejected. The observed Type&nbsp;I error rates remained close to the nominal levels, confirming the validity and proper calibration of the proposed procedures.
...
This layout serves as a base template to generate the simulated image shown in Figures&nbsp;\ref{fig:sim_Phantom_2}.

Each region is filled with data simulated from the $\mathcal{G}^0_I$ distribution&nbsp;\eqref{E:gi02}, using different combinations of the texture parameter $\alpha$ and the mean $\mu$, as indicated by the labels in each quadrant of the image.
...
\begin{figure*}[hbt]
   ...
   \hspace{0.000001\textwidth}
    \begin{subfigure}{0.31\textwidth}
        ...
        \label{fig:pvalueL9}
    \end{subfigure}
    \caption{Results on simulated data with $S_{\widetilde{T}_{\lambda}}$, using adaptive windows ($5\times5$ to $11\times11$).}
...
\begin{figure*}[!t]
   ...
   \hspace{0.01\textwidth}
    \begin{subfigure}{0.34\textwidth}
        ...
        \label{fig:munichp}
    \end{subfigure}
    \caption{Results on SAR data over Munich, Germany, using the test statistic $S_{\widetilde{T}_{\lambda}}$ with adaptive windows ($5\times5$ to $11\times11$).}
    \label{fig:munich}
\end{figure*}
```
...
\begin{figure*}[!t]
   ...
   \hspace{0.01\textwidth}
    \begin{subfigure}{0.335\textwidth}
        ...
        \label{fig:dublinp}
    \end{subfigure}
   \caption{Results on SAR data over Dublin, Ireland, using the test statistic $S_{\widetilde{T}_{\lambda}}$ with adaptive windows ($5\times5$ to $11\times11$).}
    \label{fig:dublin}
\end{figure*}
```
...
\begin{figure*}[!t]
   ...
   \hspace{0.01\textwidth}
    \begin{subfigure}{0.33\textwidth}
        ...
        \label{fig:foggiap}
    \end{subfigure}
   \caption{Results on SAR data over Foggia, Italy, using the test statistic $S_{\widetilde{T}_{\lambda}}$ with adaptive windows ($5\times5$ to $11\times11$).}
    \label{fig:foggia}
\end{figure*}
```
