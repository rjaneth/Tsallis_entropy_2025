...
[S]{.IEEEPARstart}ynthetic Aperture Radar (SAR) technology has become a
...
which enables continuous and reliable Earth observation [@Mondini2021].
SAR images are widely applied in environmental
monitoring [@Amitrano2021], disaster assessment [@Ge2020],
agriculture [@Changan2019], urban planning [@Esch2010;
@NovelTechniquesforBuiltupAreaExtractionfromPolarimetricSARImages2019],
and maritime surveillance [@Cao2024;
@OntheDetectionandLongTermPathVisualisationofa68Iceberg], among other
fields [@Moreira2013]. However, the effective use of SAR data is
hindered by speckle, a noise-like granular interference inherent to
coherent imaging systems [@Argenti2013]. Speckle can be described as
multiplicative, non-Gaussian noise, making statistical modeling and
image interpretation particularly challenging [@Baraha2023].
...
homogeneous regions [@Frery1997; @Ferreira2020]. While these models are
effective, parameter estimation becomes unstable when applied to small
local windows, which are often required to preserve spatial resolution.
This limitation motivates alternative methodologies that avoid heavy
reliance on parametric estimation.

Entropy has long been used to quantify the uncertainty of a random
variable, with Shannon entropy [@Shannon1948] as the classical form.
Recent studies have shown the value of entropy-based methods in tasks
such as edge detection [@Nascimento2014], segmentation [@Nobre2016], and
despeckling [@Chan2022], motivating their extension to hypothesis
testing in SAR imagery. In this realm, Frery et al. [@Frery2024] and
Alpala et al. [@Alpala2025] quantified departures from the
fully-developed speckle hypothesis with the Shannon and Rényi entropy,
respectively.

The Tsallis entropy [@Tsallis1988] offers a non-additive generalization
that introduces a parameter $\lambda$, allowing greater flexibility in
capturing deviations from homogeneity and better sensitivity to
heavy-tailed data. This makes it especially appealing for SAR analysis,
where both homogeneous and highly heterogeneous areas coexist.
...
method of Park et al. [@Park1999]. Instead of relying on fixed-size
sliding windows (e.g., $7 \times 7$), the adaptive procedure adjusts the
window size locally: it enlarges in homogeneous regions to improve
statistical stability, and shrinks in heterogeneous regions to avoid
mixing different structures and to preserve local details.

The proposed method is unsupervised, does not require training data, and
produces interpretable $p$-value maps, making it suitable for scenarios
where ground-truth information is scarce or unavailable.
...
heterogeneity [@Frery1997].

We denote $Z \sim \Gamma_{\mathrm{SAR}}(\mu,L)$ when the random variable
$Z$ follows the Gamma SAR distribution, whose probability density
function (pdf) is: \begin{equation}
...
\end{equation} Similarly, $Z \sim \mathcal{G}^0_I(\alpha,\gamma,L)$ has
density: \begin{align}
...
of the $\mathcal{G}^0_I$ distribution, $L \geq 1$ is the number of looks
(either nominal or estimated, thus not restricted to integer values),
$\Gamma(\cdot)$ is the gamma function, and $\mathbbm 1_{A}(\cdot)$ is
...
Therefore, assuming $\alpha<-1$, its expected value is \begin{equation}
    \mu=\left(\frac{\gamma}{L}\right)\frac{\Gamma(-\alpha-1)}{\Gamma(-\alpha)}\frac{\Gamma(L+1)}{\Gamma(L)}=-\frac{\gamma}{\alpha+1}.
    \label{E:mean1}
\end{equation}

Although the $\mathcal{G}^0_I$ distribution is defined by the parameters
$\alpha$ and $\gamma$, in the SAR literature [@Nascimento2010] the
texture parameter $\alpha$ and the mean $\mu$ are commonly used.
Expressing $\gamma$ in terms of $\mu$ as $\gamma=-\mu(\alpha+1)$
in \eqref{E:gi01}, and denoting the reparametrized model as
$Z \sim \mathcal{G}^0_I(\alpha,\mu,L)$, we obtain for $\alpha<-1$ (which
ensures $\gamma>0$ and that the mean $\mu$ exists): \begin{multline}
...
\end{multline} The $\Gamma_{\mathrm{SAR}}$ model is a particular case of
the $\mathcal{G}^0_I$ distribution [@Frery1997]. Specifically, for fixed
$\mu$, $$
...
Charvát [-@Havrda1967] in the context of information theory, and later
extended by Tsallis [-@Tsallis1988] to emphasize its non-extensive
features in statistical physics.
...
Tsallis entropy of the $\Gamma_{\mathrm{SAR}}$ and the $\mathcal{G}^0_I$
distributions:
...
\end{equation*} where the term $\Delta_\alpha$ measures the increase in
entropy induced by the texture level, with the level controlled by the
parameter $\alpha$. As $\alpha \to -\infty$ (fully developed speckle),
$\Delta_\alpha \to 0$ and \eqref{eq:GI0-Tsallis} reduces to the
homogeneous case in \eqref{eq:gammasar-Tsallis}, \emph{i.e.}, $$
\lim_{\alpha \to -\infty} T_{\lambda}\bigl(\mathcal{G}^0_I(\mu, \alpha, L)\bigr) =  T_{\lambda}\bigl(\Gamma_{\mathrm{SAR}}(\mu, L)\bigr).
$$

@fig-tsallis-convergence_L18 illustrates the convergence of the Tsallis
entropy for different values of $\alpha$ and $L \in \{5,18\}$. In every
case, the dashed curves of $T_\lambda(\mathcal{G}^0_I)$ approach the
solid black curve of $T_\lambda(\Gamma_{\mathrm{SAR}})$, confirming the
limiting behavior.
...
For the highly heterogeneous case ($\alpha=-2$), entropy values are very
similar for $L=5$ and $L=18$, since texture dominates over speckle. In
contrast, in the homogeneous limit ($\alpha \to -\infty$), texture
effects vanish and entropy is mainly determined by speckle. With a low
number of looks, speckle is strong and increases the uncertainty, which
raises the entropy values. However, the curves for different $\alpha$
values appear more concentrated, since the contribution of texture to
entropy is masked by the dominant speckle. As the number of looks
increases, speckle is progressively reduced and entropy decreases. This
reduction of speckle makes the entropy excess $\Delta_\alpha$ more
evident, resulting in more separated curves across $\alpha$.

## Nonparametric Estimation of Tsallis Entropy

\label{subsec:tsallis-np}

Following the spacing-based approach of Vasicek [-@vasicek1976test] and
Ebrahimi et al. [-@Ebrahimi1994] for Shannon entropy, and the
methodology for Rényi entropy proposed by Al-Labadi et
al. [-@AlLabadi2024], we derive a nonparametric estimator for Tsallis
entropy.

Let $Q(p)=F^{-1}(p)$ denote the quantile function for $p\in(0,1)$. From
...
$\mathrm{d}p=f(z)\,\mathrm{d}z$) and the identity $f(Q(p))=1/Q'(p)$, we
obtain $$
  \mathbb{E}\!\left[f(Z)^{\lambda-1}\right]
...
\end{equation} Let $\bm{Z}=(Z_1,Z_2,\dots,Z_n)$ be an i.i.d. sample from
$F$, with order statistics $Z_{(1)}\le Z_{(2)} \le \cdots\le Z_{(n)}$.
For a window size $m\in\{1,\dots,\lfloor (n-1)/2\rfloor\}$, define the
$m$-spacing $$
...
$\widehat{Q'}(p_i)$ from \eqref{eq:Qprime-hat} yields the nonparametric
Tsallis entropy estimator: \begin{align}
...
\end{align} As $\lambda\to 1$, $T_\lambda$ converges to Shannon entropy
and $\widehat T_{\lambda}$ recovers the Vasicek
estimator [-@vasicek1976test]. This estimator is asymptotically
consistent, \emph{i.e.}, it converges in probability to the true value
$T_\lambda$ when $m,n\rightarrow\infty$ and $m/n\rightarrow0$. We use
the heuristic spacing $m=\lfloor\sqrt{n}+0.5\rfloor$.
...
that $\lambda=0.85$ yields the lowest MSE while maintaining a low bias,
thus providing a favorable trade-off between bias and variance.
...
Following Refs. [@Frery2024; @Alpala2024; @Alpala2025], we refine the
nonparametric entropy estimator $\widehat{T}_{\lambda}$ defined
...
$\bm{Z}^{(b)}$ denotes the $b$-th resampled dataset obtained by drawing
$n$ observations with replacement from $\bm{Z}$.

A Monte Carlo study with $1000$ replications for each sample size
$n \in \{9, 25, 49, 81, 121\}$ from the $\Gamma_{\mathrm{SAR}}$ (1,5)
...
significant improvements for small samples, as shown in
...
```{r fig-bias_mse_Tsallis,  echo=FALSE, message=FALSE, warning=FALSE,  out.width=".9\\linewidth",  fig.pos="H",  fig.cap="Bias and MSE for the Tsallis entropy estimators of $\\Gamma_{\\mathrm{SAR}}$, with $L=5$", fig.asp=0.75}
...
be based on the improved bootstrap estimator $\widetilde{T}_{\lambda}$.

## Hypothesis Testing

We test whether the observed data come from a homogeneous
($\Gamma_{\mathrm{SAR}}$) or a heterogeneous ($\mathcal{G}^0_I$) region,
as follows: \begin{equation}\label{eq:hypothesis_test}
\begin{cases}
...
$n \in \{25, 49, 81,121\}$, $\lambda = 0.85$, and $L \in \{5,18\}$. The
empirical densities are tightly concentrated around zero, confirming
...
```{r fig-densities-tsallis6,  echo=FALSE, message=FALSE, warning=FALSE,  out.width="\\linewidth", fig.pos="H",  fig.cap="Empirical densities of $S_{\\widetilde{T}_{\\lambda}}(\\bm{Z}; L)$ under $\\mathcal{H}_0$, with $\\lambda=0.85$."}
#fig.show="hide", 
...
[@vasicek1976test; @VanEs1992], for sufficiently large samples,
$S_{\widetilde{T}}(\bm{Z};L)$ follows an asymptotic Normal distribution,
\emph{i.e.}, the following convergence in distribution holds: $$
...
$n$. In practice, we estimate $\widehat{\mu}_S$ and $\widehat{\sigma}_S$
via Monte Carlo under $\mathcal{H}_0$. Therefore, \begin{equation*}
...
it using the estimated mean $\widehat{\mu}_S$ and the standard deviation
$\widehat{\sigma}_S$; next, compute the $p$-value as
...
$\Gamma_{\mathrm{SAR}}$ distribution with $\mu=1$, computing the test
statistic via a bootstrap entropy estimator using $B=100$ and
$\lambda=0.85$. The empirical size was then estimated as the proportion
of replications in which the null hypothesis was incorrectly rejected.
The observed Type I error rates remained close to the nominal levels,
confirming the validity and proper calibration of the proposed
procedures.
...
strong sensitivity to departures from $\mathcal{H}_0$. To assess power,
we simulated data under the alternative hypothesis $\mathcal{H}_1$,
assuming a $\mathcal{G}^0_I$ distribution with $\mu=1$ and $\alpha=-2$.
...
bootstrap estimator with $B=100$ and $\lambda=0.85$. As expected, power
increased with both the sample size and the number of looks, confirming
the effectiveness of the Tsallis entropy–based test in detecting
...
proposed by Park et al. [@Park1999], which adjusts the analysis window
size according to the local homogeneity of the scene. For each pixel
$(i,j)$, let $w_{ij}$ be the window centered at $(i,j)$ with radius
$N_{ij}$, defined as \begin{equation}
...
$\sigma_{ij}$ are calculated using only the border pixels $b_{ij}$ of
$w_{ij}$.
...
smoothing. Values close to $\eta=1$ provide limited speckle suppression,
...
($C_{ij} \le U_{ij}$), the window grows (up to $N_{\max}$), enhancing
speckle reduction and stabilizing the estimates; in heterogeneous
regions ($C_{ij} > U_{ij}$), the window shrinks (down to $N_{\min}$),
preserving edges and preventing class mixing.

Once the window radius $N_{ij}$ has been determined for pixel $(i,j)$,
the corresponding full window is extracted and the proposed test in
\eqref{eq-test-tsallis} is applied to obtain the local test statistic.
This produces two outputs: (i) a map of test statistics over the image,
and (ii) a window-size map $W_{ij}$ that records the locally selected
window side (larger in homogeneous regions, smaller in heterogeneous
ones). For inference, each value of the test statistic is standardized
using the empirical mean and standard deviation of the image, and
two-sided $p$-values are then computed.
...
the parameter $\eta$ controls the balance between smoothness and detail
preservation. Moreover, the framework is extensible and can be applied
with any entropy estimator.
...
<!-- $500\times500$ pixels, proposed by  Gomez et al.&nbsp;[@Gomez2017] as a tool to assess the performance of speckle-reduction filters.  -->

<!-- This layout serves as a base template to generate the simulated image shown in Figure&nbsp;\ref{fig:sim_Phantom_2}. -->

Figure \ref{fig:sim_scene} shows the simulated image of size
$500\times500$ pixels, generated from the structural design proposed by
Gomez et al. [@Gomez2017] as a tool to assess the performance of
speckle-reduction filters. Each region is filled with simulated samples
drawn from the $\mathcal{G}^0_I$ distribution using the
expression \eqref{E:gi02}, considering different combinations of the
...
zones [@Frery2019a]. As the value decreases, it indicates regions with
moderate texture (in the $\left[-6,-3\right]$ region), related to forest
zones, while values below $-6$ correspond to textureless regions, such
as pasture, agricultural fields, and water bodies [@Neto2023].
...
with side lengths ranging from $5\times5$ ($N_{\min}=5$) to $11\times11$
($N_{\max}=11$). For comparison, a fixed $7\times7$ window was also
considered.
...
an interpretable scale: dark red regions ($p \approx 0$) indicate strong
evidence of heterogeneity, consistently appearing in highly textured
zones, while dark blue regions ($p \approx 1$) correspond to homogeneous
areas. Intermediate colors follow a smooth progression: orange
($p\!\approx\!0.2$–$0.4$) denotes transitional texture, whereas
yellow–green to cyan ($p\!\approx\!0.4$–$0.8$) marks increasingly
...
$\text{ENL} = 1/\widehat{\text{CV}}^2$, based on the sample coefficient
of variation. The results were consistent with the metadata. We also
verified that moderate deviations in $L$ did not significantly impact
the outcome of the test, confirming the robustness of the proposed
method.

## Qualitative analysis

The proposed Tsallis-based test statistic, $S_{\widetilde{T}{\lambda}}$,
was applied to four SAR intensity images. For each scene, results were
obtained using a fixed $7 \times 7$ sliding window and the adaptive
windowing scheme with side lengths ranging from $5\times5$
($N_{\min}=5$) to $11\times11$ ($N_{\max}=11$), for smoothing parameters
$\eta=1.0$, $2.0$, and $3.0$.

Figures \ref{fig:sanfrancisco}–\ref{fig:foggia} present the outcomes,
organized in two rows: the first row shows the $p$-value heatmaps, while
the second row displays the corresponding binary decision maps at the
$0.05$ significance level, where black pixels denote heterogeneous areas
and white pixels homogeneous regions.
...
<!-- In all scenes, the adaptive strategy improves the detection of textured features.  -->

In the $p$-value heatmaps, dark-red tones ($p \approx 0$) concentrate on
...
The effect of the number of looks is evident. In San Francisco ($L=4$),
where speckle is stronger, adaptive maps, especially for $\eta=2.0$ and
$\eta=3.0$, enhance the detection of urban structures compared to the
fixed window.
...
Visual inspection suggests that $\eta=2.0$ and $\eta=3.0$ yield the most
balanced results. With $\eta=1.0$, the outcome is close to the
fixed-window case, offering little improvement. Values above $\eta=3.0$
tend to oversmooth the images, causing blurred edges and loss of fine
details. These findings highlight the advantages of the adaptive
approach, which better preserves heterogeneous features while enhancing
the clarity of homogeneous areas, consistent with the behavior observed
