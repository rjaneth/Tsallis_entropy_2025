...
%DIF DEL R0-Tsallis_entropy_sar.tex   Thu Feb 26 04:19:59 2026
%DIF ADD R1-Tsallis_entropy_sar.tex   Tue May 12 14:36:31 2026
...
%DIFDELCMD < 50670-901 PE, Brazil (e-mail: janeth.alpala@ufpe.br, abraao@de.ufpe.br
%DIFDELCMD < ).
%DIFDELCMD < 
...
%DIFDELCMD < 
%DIFDELCMD < Paolo Gamba is with the Department of Electrical, Computer and
%DIFDELCMD < Biomedical Engineering, University of Pavia, 27100 Pavia, Italy (e-mail:
%DIFDELCMD < paolo.gamba@unipv.it)
%DIFDELCMD < 
%DIFDELCMD < Andrea A. Rey is with the Laboratorio de Investigación y Desarrollo
%DIFDELCMD < Experimental en Computación, Universidad Nacional de Hurlingham
%DIFDELCMD < (UNAHUR), Villa Tesei, 1688, Argentina (e-mail:
%DIFDELCMD < andrea.rey@unahur.edu.ar).}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \thanks{Janeth Alpala is with the Departamento de Estatística,
...
framework that generates per-pixel \DIFdelbegin \DIFdel{\(p\)}\DIFdelend \DIFaddbegin \DIFadd{\(\bm p\)}\DIFaddend -value maps for detecting
...
heterogeneous regions. The proposed approach offers \DIFdelbegin \DIFdel{a principled,
}\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend interpretable
tool for automated SAR image analysis without requiring training data or
\DIFdelbegin \DIFdel{scene-specific calibration}\DIFdelend \DIFaddbegin \DIFadd{explicit parametric texture estimation}\DIFaddend .
\end{abstract}
...
\DIFaddend \renewcommand{\tablename}{TABLE}

\section{Introduction}\label{introduction}

\DIFdelbegin %DIFDELCMD < \IEEEPARstart{S}%%%
\DIFdel{ynthetic }\DIFdelend \DIFaddbegin \IEEEPARstart{S}{ynthetic} \DIFaddend Aperture Radar (SAR) technology is a key tool
...
\DIFaddbegin \DIFadd{Deep learning has driven significant advances in SAR image analysis. In
...
\DIFaddend Among the statistical laws used for SAR intensities, the
\(\mathcal{G}_I^0\) distribution~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Frery1997}{{[}12{]}}}\hskip0pt%DIFAUXCMD
,
\mbox{%DIFAUXCMD
\citeproc{ref-Ferreira2020}{{[}13{]}} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Frery1997}{{[}16{]}}}\hskip0pt%DIFAUXCMD
,
\mbox{%DIFAUXCMD
\citeproc{ref-Ferreira2020}{{[}17{]}} }\hskip0pt%DIFAUXCMD
}\DIFaddend is widely recognized for its
...
variable, with Shannon entropy~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Shannon1948}{{[}14{]}} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Shannon1948}{{[}18{]}} }\hskip0pt%DIFAUXCMD
}\DIFaddend as
the classical form. Recent studies have shown the value of entropy-based
methods in tasks such as edge
detection~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Nascimento2014}{{[}15{]}}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Nascimento2014}{{[}19{]}}}\hskip0pt%DIFAUXCMD
}\DIFaddend ,
segmentation~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Nobre2016}{{[}16{]}}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Nobre2016}{{[}20{]}}}\hskip0pt%DIFAUXCMD
}\DIFaddend , and
despeckling~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Chan2022}{{[}17{]}}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Chan2022}{{[}21{]}}}\hskip0pt%DIFAUXCMD
}\DIFaddend , motivating their
extension to hypothesis testing in SAR imagery. In this domain, Frery et
al.~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Frery2024}{{[}18{]}} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Frery2024}{{[}22{]}} }\hskip0pt%DIFAUXCMD
}\DIFaddend and Alpala et
al.~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Alpala2025}{{[}19{]}} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Alpala2025}{{[}23{]}} }\hskip0pt%DIFAUXCMD
}\DIFaddend quantified departures from the
fully-developed speckle hypothesis with the Shannon and Rényi entropy,
respectively.

The Tsallis entropy~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Tsallis1988}{{[}20{]}} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Tsallis1988}{{[}24{]}} }\hskip0pt%DIFAUXCMD
}\DIFaddend is a
...
In this work, we propose a \DIFdelbegin \DIFdel{statistical }\DIFdelend \DIFaddbegin \DIFadd{method that exploits the statistical
...
\DIFadd{The resulting approach requires no retraining or hyperparameter tuning,
does not depend on sensors and resolutions, and yields fully
interpretable results. The method offers an alternative when labeled
data are scarce, interpretability is required, or computational
resources are constrained. It can serve as a feature layer within
explainable machine learning pipelines.
}

\DIFadd{Our proposal revolves around a statistical }\DIFaddend test based on a nonparametric
...
\citeproc{ref-Park1999}{{[}21{]}}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Park1999}{{[}25{]}}}\hskip0pt%DIFAUXCMD
}\DIFaddend . Instead of relying on
...
(\(\Gamma_{\mathrm{SAR}}\))~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Lampropoulos1999}{{[}22{]}}}\hskip0pt%DIFAUXCMD
,
\mbox{%DIFAUXCMD
\citeproc{ref-Li2011}{{[}23{]}}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Lampropoulos1999}{{[}26{]}}}\hskip0pt%DIFAUXCMD
,
\mbox{%DIFAUXCMD
\citeproc{ref-Li2011}{{[}27{]}}}\hskip0pt%DIFAUXCMD
}\DIFaddend , a reparametrization of the classical
...
heterogeneity~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Frery1997}{{[}12{]}}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Frery1997}{{[}16{]}}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
...
\(\mu\)~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Nascimento2024}{{[}24{]}}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Nascimento2024}{{[}28{]}}}\hskip0pt%DIFAUXCMD
}\DIFaddend . Expressing \(\gamma\)
...
the \(\mathcal{G}^0_I\) distribution~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Frery1997}{{[}12{]}}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Frery1997}{{[}16{]}}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
...
Charvát~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Havrda1967}{{[}25{]}} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Havrda1967}{{[}29{]}} }\hskip0pt%DIFAUXCMD
}\DIFaddend in the context of
information theory, and later extended by
Tsallis~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Tsallis1988}{{[}20{]}} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Tsallis1988}{{[}24{]}} }\hskip0pt%DIFAUXCMD
}\DIFaddend to emphasize its
...
\end{equation*} where the term \(\Delta_\alpha>0\) measures the increase
...
the homogeneous case in~\eqref{eq:gammasar-Tsallis}, i.e., \[
...
%DIFDELCMD < \pandocbounded{\includegraphics[keepaspectratio]{Figures/fig-tsallis-convergence_L18-1.pdf}}
%DIFDELCMD < 
...
\pandocbounded{\includegraphics[keepaspectratio]{Figures-R1/fig-tsallis-convergence_L18-1.pdf}}
...
\citeproc{ref-vasicek1976test}{{[}26{]}} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-vasicek1976test}{{[}30{]}} }\hskip0pt%DIFAUXCMD
}\DIFaddend and Ebrahimi et
al.~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Ebrahimi1994}{{[}27{]}} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Ebrahimi1994}{{[}31{]}} }\hskip0pt%DIFAUXCMD
}\DIFaddend for the Shannon entropy, and
the methodology for the Rényi entropy proposed by Al-Labadi et
al.~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-AlLabadi2024}{{[}28{]}}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-AlLabadi2024}{{[}32{]}}}\hskip0pt%DIFAUXCMD
}\DIFaddend , we derive a nonparametric
...
estimator~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-vasicek1976test}{{[}26{]}}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-vasicek1976test}{{[}30{]}}}\hskip0pt%DIFAUXCMD
}\DIFaddend . While a formal proof
...
\mbox{%DIFAUXCMD
\citeproc{ref-Ebrahimi1994}{{[}31{]}} }\hskip0pt%DIFAUXCMD
}\DIFaddend and for the Rényi
entropy~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-AlLabadi2024}{{[}28{]}}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-AlLabadi2024}{{[}32{]}}}\hskip0pt%DIFAUXCMD
}\DIFaddend . Empirically,
...
Ref.~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Crzcgorzewski1999}{{[}29{]}}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Crzcgorzewski1999}{{[}33{]}}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
...
%DIFDELCMD < \includegraphics[width=0.9\linewidth,height=\textheight,keepaspectratio]{Figures/fig-optimal_order-tsallis-1.pdf}
%DIFDELCMD < 
...
\DIFaddbegin \DIFadd{There is an interpretation for this optimal value of \(\lambda\). When
considering discrete probabilities, smaller values this entropic index
amplify the effect of ``small'' probabilities on the total Tsallis
...
\DIFaddend \subsection{Bootstrap Correction for Entropy
...
,
\mbox{%DIFAUXCMD
\citeproc{ref-Alpala2024}{{[}30{]}}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Frery2024}{{[}22{]}}}\hskip0pt%DIFAUXCMD
,
\mbox{%DIFAUXCMD
...
,
\mbox{%DIFAUXCMD
\citeproc{ref-Alpala2024}{{[}34{]}}}\hskip0pt%DIFAUXCMD
}\DIFaddend , we refine the nonparametric entropy
...
%DIFDELCMD < \includegraphics[width=0.9\linewidth,height=\textheight,keepaspectratio]{Figures/fig-bias_mse_Tsallis-1.pdf}
%DIFDELCMD < 
...
\DIFaddbegin \DIFadd{Heterogeneous samples occur when the window spans two or more different
...
almost \mbox{%DIFAUXCMD
\SI{80}{\percent} }\hskip0pt%DIFAUXCMD
of the observations when reducing from a window
of size \(11\times11\) pixels to another of size \(5\times5\) pixels.
}

\DIFaddend \subsection{Hypothesis Testing}\label{hypothesis-testing}
...
%DIFDELCMD < \pandocbounded{\includegraphics[keepaspectratio]{Figures/fig-densities-tsallis6-1.pdf}}
%DIFDELCMD < 
...
\pandocbounded{\includegraphics[keepaspectratio]{Figures-R1/fig-densities-tsallis6-1.pdf}}
...
estimators~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-vasicek1976test}{{[}26{]}}}\hskip0pt%DIFAUXCMD
,
\mbox{%DIFAUXCMD
\citeproc{ref-VanEs1992}{{[}31{]}}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-vasicek1976test}{{[}30{]}}}\hskip0pt%DIFAUXCMD
,
\mbox{%DIFAUXCMD
\citeproc{ref-VanEs1992}{{[}35{]}}}\hskip0pt%DIFAUXCMD
}\DIFaddend , for sufficiently large samples,
...
\citeproc{ref-MathematicalStatisticsvII2015}{{[}32{]}}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-MathematicalStatisticsvII2015}{{[}36{]}}}\hskip0pt%DIFAUXCMD
}\DIFaddend , Ch. 7), its
...
\begin{table}[hbt]
\centering\centering
\caption{\label{tab:size-power-tsallis}Size and Power of the $S_{\widetilde{T}_{\lambda}}(\bm{Z})$ test statistic\DIFaddbeginFL \DIFaddFL{.}\DIFaddendFL }
...
\subsection{Adaptive Windows}\DIFdelbegin %DIFDELCMD < \label{adaptive-windows}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{sec:adaptive}
\DIFaddend 
...
statistic may overlap areas with different distribution\DIFaddbegin \DIFadd{s}\DIFaddend  . Intuitively,
...
proposed by Park et al.~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Park1999}{{[}21{]}}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Park1999}{{[}25{]}}}\hskip0pt%DIFAUXCMD
}\DIFaddend , which adjusts
...
adaptive threshold \(U_{ij}\) given by: \DIFdelbegin \begin{displaymath}
\DIFdel{U_{ij} = \eta\!\left( 1 + \sqrt{\frac{1 + 2\sigma_n^2}{8\,(W_{ij} - 1)}} \right)\sigma_n,
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation*}
\DIFadd{U_{ij} = \eta\!\left( 1 + \sqrt{\frac{1 + 2\sigma_n^2}{8\,(W_{ij} - 1)}} \right)\sigma_n,
}\end{equation*}\DIFaddend  where \(W_{ij} = 2N_{ij} + 1\) is the current window
...
In homogeneous regions\DIFdelbegin \DIFdel{(\(C_{ij} \le U_{ij}\)), }\DIFdelend \DIFaddbegin \DIFadd{, where \(C_{ij}\leq U_{ij}\), }\DIFaddend the window radius
increases \DIFdelbegin \DIFdel{(}\DIFdelend up to \(N_{\max}\)\DIFdelbegin \DIFdel{)}\DIFdelend , enhancing speckle reduction and
stabilizing the estimates\DIFdelbegin \DIFdel{; in heterogeneous regions(\(C_{ij} > U_{ij}\)), }\DIFdelend \DIFaddbegin \DIFadd{. In heterogeneous regions, where
\(C_{ij}>U_{ij}\), }\DIFaddend the window radius decreases \DIFdelbegin \DIFdel{(}\DIFdelend down to \(N_{\min}\)\DIFdelbegin \DIFdel{)}\DIFdelend ,
...
in \DIFdelbegin \DIFdel{~}\DIFdelend \eqref{eq-test-tsallis} is applied to obtain a local test value. This
produces a map of \DIFdelbegin \DIFdel{test }\DIFdelend statistics over the image.
\DIFdelbegin \DIFdel{For inference, each
value in this map is standardized using the empirical mean
}\DIFdelend \DIFaddbegin 

\DIFadd{Because the final window size \(W_{ij}\) is selected from the data, the
...
\subsection{\DIFadd{Test statistic behavior under the null hypothesis and
adaptive windows}}\label{sec:adaptive-null}

\DIFadd{Table~\ref{tab:size-power-tsallis} relates the behavior of the test
statistic under the null hypothesis for windows of fixed size. Since the
procedure is adaptive, the final window size \(W_{ij}\) at position
\((i,j)\) is selected from the data, and the test statistic behavior may
be different. We verified that these distributions differ, and obtained
the \(p\)-values under this realistic configuration.
}

\DIFadd{For this purpose, we generated an image of size \(1000\times1000\) under
...
\DIFadd{Table~\ref{tbl-null-calibration-adaptive} reports summary statistics
separately for each admissible window size \(W\): the mean
\(\mu_{\mathcal H_0}(W)\) }\DIFaddend and standard deviation
\DIFdelbegin \DIFdel{of the image, and the two-sided }\DIFdelend \DIFaddbegin \DIFadd{\(\sigma_{\mathcal H_0}(W)\). With these values, we corrected the
empirical distribution of the test statistic, cf.~\eqref{eq:standar},
and obtained }\DIFaddend \(p\)\DIFdelbegin \DIFdel{-value is then computed}\DIFdelend \DIFaddbegin \DIFadd{-values adjusted for the adaptive window scheme}\DIFaddend .

\DIFdelbegin \DIFdel{This adaptive strategy is locally responsive to image texture, using
small windows in heterogeneous areas and larger ones in homogeneous
zones.
This ensures edge preservation and statistical stability, while
the parameter \(\eta\) controls the balance between smoothness and detail preservation. Moreover, the framework is extensible and can be
applied with any entropy estimator}\DIFdelend \DIFaddbegin \begin{table}

\caption{\label{tbl-null-calibration-adaptive}\DIFaddFL{Empirical mean and standard deviation of the test statistic under the null 
...
\DIFadd{Figure~\ref{fig:null-diagnostic-adaptive} shows the empirical densities
...
never selected windows of size \(W=5\)}\DIFaddend .

\DIFaddbegin \begin{figure}[hbt]
  \centering
  \includegraphics[width=0.98\columnwidth]{Figures-R1/distributions_statistic_density_1.pdf}
 \caption{\DIFaddFL{Empirical densities of the test statistic under the null hypothesis and fixed and adaptive window sizes.}}
  \label{fig:null-diagnostic-adaptive}
\end{figure}

\DIFadd{After establishing the mean and standard deviation of the test statistic
under the null hypothesis and adaptive windows, the next step is to
convert the statistic field into \(p\)-values.
}

\DIFadd{Since \(\sigma_{\mathcal H_0}(W)\) produces overly aggressive
standardization and excessive rejections, we used }\begin{equation}
\DIFadd{z_{ij}=\frac{S_{ij}-\mu_{\mathcal H_0}(W_{ij})}{\sigma_{\mathrm{obs}}},
\label{eq:standar}
}\end{equation} \DIFadd{where \(\sigma_{\mathrm{obs}}\) is the empirical standard
deviation of the test statistic computed over all the pixels under
\(\mathcal H_0\) using adaptive windows. In this way, the procedure uses
window-dependent means and a scale better matched to the variability of
the observed SAR data. Finally, the adaptive \(p\)-values are computed
as }\[
\DIFadd{p_{ij}=2\Phi\bigl(-|z_{ij}|\bigr).
}\]

\DIFadd{The same strategy was subsequently used in the comparative benchmark for
the adaptive Shannon and Rényi entropy-based tests, as discussed in
Subsection~\ref{sec:entropy-benchmark}.
}

\DIFaddend \section{RESULTS}\label{sec:app}

In this section, we present the \DIFdelbegin \DIFdel{obtained results of applying our
proposal }\DIFdelend \DIFaddbegin \DIFadd{results obtained by applying the
proposed method }\DIFaddend to simulated data and \DIFdelbegin \DIFdel{to }\DIFdelend actual SAR images.
...
al.~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Gomez2017}{{[}33{]}} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Gomez2017}{{[}37{]}} }\hskip0pt%DIFAUXCMD
}\DIFaddend as a tool to assess the
...
zones~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Frery2019a}{{[}34{]}}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Frery2019a}{{[}38{]}}}\hskip0pt%DIFAUXCMD
}\DIFaddend . As the \(\alpha\) value
...
and water bodies~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Neto2023}{{[}35{]}}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Neto2023}{{[}39{]}}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
...
\begin{table}[hbt]
\centering\centering
\caption{\label{tab:table_param}Parameters of selected SAR images\DIFaddbeginFL \DIFaddFL{.}\DIFaddendFL }
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{24}{26}\selectfont
\begin{tabu} to \linewidth {>{\centering\arraybackslash}p{4.0cm}>{\centering\arraybackslash}p{4.3cm}>{\centering\arraybackslash}p{1.5cm}>{\centering\arraybackslash}p{4cm}>{\centering\arraybackslash}p{1.5cm}>{\centering\arraybackslash}p{4.0cm}>{\centering\arraybackslash}p{4.0cm}}
\toprule
...
\caption{$p$-value \DIFaddbeginFL \DIFaddFL{map }\DIFaddendFL ($\eta=4$)}
\label{fig:SFc5}
\end{subfigure}
...
visible advantages: homogeneous areas become smoother while fine
structures such as roads and field boundaries remain visible. Visual
inspection suggests that \(\eta\) values between \(2\) and \(3\) provide
the most balanced results. With \(\eta = 1\), the outcome is close to
the fixed-window case, offering little improvement, whereas \(\eta = 4\)
tends to oversmooth the images, causing blurred edges and loss of fine
details (see panels (j) in Figures~\ref{fig:munich}--\ref{fig:foggia}).
...
detection using fixed and adaptive windows\DIFaddbeginFL \DIFaddFL{.}\DIFaddendFL }
...
moderate computational cost \DIFaddbegin \DIFadd{(the running time increases by, at most, a
factor of \(15\))}\DIFaddend . Stable results can already be achieved with
...
heterogeneity detection\DIFaddbeginFL \DIFaddFL{.}\DIFaddendFL }
...
\DIFaddbegin \subsection{\DIFadd{Comparative Benchmark Between Entropy-Based
Tests}}\label{sec:entropy-benchmark}

\DIFadd{To compare the proposed Tsallis-based test with earlier entropy-based
methods for SAR imagery, we carried out a benchmark against the Shannon
and Rényi-based tests. In particular, the Shannon-based test
in~\mbox{%DIFAUXCMD
\citeproc{ref-Frery2024}{{[}22{]}} }\hskip0pt%DIFAUXCMD
and the Rényi-based test
in~\mbox{%DIFAUXCMD
\citeproc{ref-Alpala2025}{{[}23{]}} }\hskip0pt%DIFAUXCMD
...
\DIFadd{The present proposal extends that line by using the Tsallis entropy,
...
\DIFadd{This comparison is relevant because the gain of the proposed method may
...
\DIFadd{For the adaptive configurations in this benchmark, we used the refined
calibration introduced in~\eqref{eq:standar}, which preserves the
window-dependent null centering while avoiding distortions that may
arise when a purely null-based scale is used in real scenes. In the
Dublin scene, this refinement produced only minor quantitative changes
and left the substantive conclusions unchanged.
}

\DIFadd{For the single-threshold summaries, we report \(F_1\), \(\kappa\), and
...
\DIFadd{Table~\ref{tbl-dublin-main-benchmark} reports the benchmark results. Two
...
Second, within both the fixed and adaptive settings, the Tsallis-based
test provides the strongest overall performance.
}

\begin{table*}

\caption{\label{tbl-dublin-main-benchmark}\DIFaddFL{Entropy-based benchmark on the Dublin SAR scene under fixed and adaptive window settings.}}
...
\DIFadd{The best configuration is the bootstrap-corrected adaptive Tsallis
...
\DIFadd{Figure~\ref{fig:roc_entropy_benchmark} provides a threshold-free
comparison through ROC curves. In both the fixed and adaptive settings,
the Tsallis-based method outperforms the Shannon and Rényi-based
alternatives, with the largest separation observed in the adaptive
bootstrap-corrected case.
}

\begin{figure}[hbt]
  \centering
  \includegraphics[width=0.99\columnwidth]{Figures-R1/dublin_entropy_benchmark_roc_2panels.pdf}
  \caption{\DIFaddFL{ROC comparison on the Dublin SAR scene for Shannon, Rényi, and Tsallis under bootstrap-corrected fixed and adaptive settings.}}
  \label{fig:roc_entropy_benchmark}
\end{figure}

\DIFadd{Figure~\ref{fig:threshold_entropy_benchmark} shows the corresponding
\(F_1\) threshold sweep for \(p\)-thresholds from \(0.01\) to \(0.20\).
The same ranking is observed across thresholds, with Tsallis performing
best, followed by Rényi and Shannon.
}

\begin{figure}[hbt]
  \centering
  \includegraphics[width=0.99\columnwidth]{Figures-R1/dublin_benchmark_f1_threshold_two_panels.pdf}
  \caption{\DIFaddFL{$F_1$ threshold sweep on the Dublin SAR scene for Shannon, Rényi, and Tsallis under bootstrap-corrected fixed and adaptive settings, using $p$-thresholds from $0.01$ to $0.20$.}}
  \label{fig:threshold_entropy_benchmark}
\end{figure}

\DIFadd{To complement the quantitative benchmark, we also performed a
...
\caption{\DIFaddFL{(a) Shannon (bootstrap + adaptive), (b) R\'enyi (bootstrap + adaptive), and (c) Tsallis (bootstrap + adaptive) on the Dublin SAR scene.}}
\label{fig:dublin_qualitative_comparison}
\end{figure*}

\DIFadd{These visual findings are consistent with the quantitative results in
Table~\ref{tbl-dublin-main-benchmark}, where the adaptive bootstrap
Tsallis configuration attained the best overall performance.
}
...
to detect \DIFdelbegin \DIFdel{deviations }\DIFdelend \DIFaddbegin \DIFadd{departures }\DIFaddend from fully-developed speckle in SAR images. The
proposed \DIFdelbegin \DIFdel{test integrates a bootstrap correction and an adaptive windowing scheme to balance speckle suppression and texture
preservation, }\DIFdelend \DIFaddbegin \DIFadd{framework combines a nonparametric entropy-based test,
bootstrap bias correction, and adaptive window selection, yielding
interpretable pixelwise \(p\)-value maps }\DIFaddend without requiring training data
or \DIFdelbegin \DIFdel{parametric modeling}\DIFdelend \DIFaddbegin \DIFadd{explicit estimation of heterogeneous parametric models}\DIFaddend .

Experiments with simulated and real SAR data showed that the method is
\DIFdelbegin \DIFdel{well calibrated, robust to small samples sizes, and effective in delineating heterogeneous regions from homogeneous ones. A Monte Carlo
validation }\DIFdelend \DIFaddbegin \DIFadd{effective in separating homogeneous and heterogeneous regions while
...
and the null diagnostic }\DIFaddend confirmed that the \DIFdelbegin \DIFdel{test maintains proper control of the Type~I~error while improving detection power with increasing sample
size. }\DIFdelend \DIFaddbegin \DIFadd{adaptive statistic remains
well behaved under homogeneous conditions. The refined adaptive
calibration based on window-dependent centering and observed-scale
standardization provided a practical and stable formulation for
real-scene inference.
}\DIFaddend 

\DIFdelbegin \DIFdel{Comparative analysis using \(p\)-value maps,
binary decisions, and
quantitative metrics, such as F1-score, Kappa coefficient, and overall
accuracy, showed that the
adaptive Tsallis-based configuration outperforms the
fixed sliding window in all evaluated scenes. The method
thus provides a reliable and interpretable tool for heterogeneity
detection in SAR imagery}\DIFdelend \DIFaddbegin \DIFadd{The comparative benchmark on a representative real SAR scene further
clarified the contribution of the proposed method with respect to
previous entropy-based approaches. Under a common evaluation protocol,
adaptive windowing improved all three entropy-based detectors, while the
bootstrap-corrected adaptive Tsallis configuration achieved the
strongest overall performance among Shannon, Rényi, and Tsallis}\DIFaddend .
...
\DIFdelend \DIFaddbegin \bibitem[\citeproctext]{ref-ORPSDOuterRectangularProjectionBasedRepresentationforOrientedShipDetectioninSARImages}
\DIFaddend \CSLLeftMargin{{[}12{]} }%
\DIFaddbegin \CSLRightInline{M. Zhang, Y. Ouyang, M. Yang, J. Guo, and Y. Li,
...
\DIFaddend \CSLRightInline{A. C. Frery, H.-J. Muller, C. C. F. Yanasse, and S. J.
...
\DIFdelend \DIFaddbegin \CSLLeftMargin{{[}29{]} }\DIFaddend %
...
\begin{IEEEbiography}
    [{\includegraphics[width=1.05in,clip]{Figures/Janeth.png}}]{\DIFadd{Janeth Alpala}}
\DIFadd{received the B.Sc. degree in Mathematics from the Universidad de Nariño, Colombia, in 2017, the M.Sc. degree in Physics and Mathematics from the Universidad de Granada, Spain, in 2019, and the Ph.D. degree in Statistics from the Universidade Federal de Pernambuco (UFPE), Recife, Brazil, in 2025.
...
\begin{IEEEbiography}
    [{\includegraphics[width=1.05in,clip]{Figures/ACF-IEEE-Face.png}}]{\DIFadd{Alejandro C. Frery}} 
    \DIFadd{(S'91--M'94--SM'14--LF'25) was born in Mendoza, Argentina, in 1960. In 1983, he received a B.Sc. in Electronic and Electrical Engineering from the Universidad de Mendoza. His M.Sc. degree was in Applied Mathematics (Statistics) from the Instituto de Matemática Pura e Aplicada (IMPA, Rio de Janeiro, 1990), and his Ph.D. degree was in Applied Computing from the Instituto Nacional de Pesquisas Espaciais (INPE, São José dos Campos, Brazil, 1993). His research interests are data visualisation, statistical computing, and stochastic modelling, with signal and image processing and networks applications.
    ...
    Prof.\ Frery is also a member of the International Society for Photogrammetry and Remote Sensing (ISPRS), the New Zealand Statistical Association, and the Statistical Society of Australia. In 2018, he received the IEEE GRSS Regional Leader Award. After serving as Associate Editor for over five years, Prof. Frery was the Editor-in-Chief of the }\textit{\DIFadd{IEEE Geoscience and Remote Sensing Letters}} \DIFadd{from 2014 to 2018. He was an IEEE Geoscience and Remote Sensing Society (GRSS) Distinguished Lecturer from 2015 to 2019. He served as this Society's AdCom (Advisory Committee) member, in charge of Future Publications, Plagiarism and Regional Symposia from 2019 to 2022, and as vice president of Publications from 2023 to 2025.
    Since 2026, he is the vice president for Meetings \& Symposia.
}\end{IEEEbiography}

\begin{IEEEbiography}
    [{\includegraphics[width=1.05in,clip]{Figures/PG.jpg}}]{\DIFadd{Paolo Gamba}}
\DIFadd{(Fellow, IEEE) is a Professor at the University of Pavia, Italy, in the Telecommunications and Remote Sensing Laboratory. He received the Laurea degree in Electronic Engineering, }\textit{\DIFadd{cum laude}}\DIFadd{, from the University of Pavia, Italy, in 1989, and the Ph.D. degree in Electronic Engineering from the same University in 1993.
Prof.\ Gamba served as Editor-in-Chief of the }\textit{\DIFadd{IEEE Geoscience and Remote Sensing Letters}} \DIFadd{from 2009 to 2013, as Editor-in-Chief of the }\textit{\DIFadd{IEEE Geoscience and Remote Sensing Magazine}} \DIFadd{from 2023 to 2024, and as Chair of the Data Fusion Committee of the IEEE Geoscience and Remote Sensing Society (GRSS) from October 2005 to May 2009. He has been elected in the GRSS AdCom from 2014 to 2022 and served as GRSS President from 2019 to 2020. He also served as Technical Co-Chair of the 2010, 2015, and 2020 IGARSS conferences, in Honolulu, HI, USA, Milan, Italy, and online, respectively.
...
\begin{IEEEbiography}
    [{\includegraphics[width=1.05in,clip]{Figures/AbraaoID.jpg}}]{\DIFadd{Abraão D. C. Nascimento}}
\DIFadd{received the B.Sc., M.Sc., and D.Sc. degrees in Statistics from the Universidade Federal de Pernambuco (UFPE), Brazil, in 2005, 2007, and 2012, respectively. Since 2014, he has been an Associate Professor with the Department of Statistics, UFPE. He currently serves as Director of the Graduate Program in Statistics at UFPE and is a researcher in Mathematics, Probability, and Statistics supported by the National Council for Scientific and Technological Development (CNPq), Brazil.
